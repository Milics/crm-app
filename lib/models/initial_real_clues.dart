import 'clue.dart';

/// 专升本 CRM 核心真实学员线索数据永久安全种子（100% 真实数据与回访时间保全）
class InitialRealClues {
  static List<Clue> getClues() {
    return [
      Clue(
        id: '1789992497246',
        wxNick: '王艺霏',
        wxId: '',
        phone: '',
        grade: '25级',
        school: '河南机电职业学院',
        subject: '美术专业综合',
        source: '老带新',
        classType: '全程非协议班',
        status: ClueStatus.enrolled,
        intentLevel: IntentLevel.high,
        tags: const ["子豪介绍"],
        nextVisitTime: DateTime.parse('2026-09-27T10:00:00.000'),
        ownerName: '超级管理员',
        createTime: DateTime.parse('2026-09-21T20:08:17.246'),
        aiAnalysisReport: null,
        aiAnalysisTime: null,
        visitLogs: [
        ],
        chatRecords: [
        ],
      ),
      Clue(
        id: '1789992432834',
        wxNick: '娄玉涵',
        wxId: '',
        phone: '',
        grade: '25级',
        school: '河南机电职业学院',
        subject: '美术专业综合',
        source: '老带新',
        classType: '全程非协议班',
        status: ClueStatus.enrolled,
        intentLevel: IntentLevel.high,
        tags: const ["子豪介绍"],
        nextVisitTime: DateTime.parse('2026-09-28T10:00:00.000'),
        ownerName: '超级管理员',
        createTime: DateTime.parse('2026-09-21T20:07:12.834'),
        aiAnalysisReport: null,
        aiAnalysisTime: null,
        visitLogs: [
        ],
        chatRecords: [
        ],
      ),
      Clue(
        id: '1789808569912',
        wxNick: '怡然自乐',
        wxId: '',
        phone: '',
        grade: '25级',
        school: '驻马店幼儿师范',
        subject: '美术专业综合',
        source: '微信',
        classType: '全程非协议班',
        status: ClueStatus.following,
        intentLevel: IntentLevel.high,
        tags: const ["家长决策", "担心二次收费"],
        nextVisitTime: DateTime.parse('2026-09-22T10:00:00.000'),
        ownerName: '超级管理员',
        createTime: DateTime.parse('2026-09-19T17:02:49.912'),
        aiAnalysisReport: null,
        aiAnalysisTime: null,
        visitLogs: [
        ],
        chatRecords: [
        ],
      ),
      Clue(
        id: '1789783775467',
        wxNick: '张博涵',
        wxId: 'zbh208128',
        phone: '',
        grade: '25级',
        school: '河南艺术',
        subject: '美术专业综合',
        source: '小红书',
        classType: '全程非协议班',
        status: ClueStatus.following,
        intentLevel: IntentLevel.medium,
        tags: const [],
        nextVisitTime: DateTime.parse('2026-09-22T10:00:00.000'),
        ownerName: '李广东',
        createTime: DateTime.parse('2026-09-19T10:09:35.467322'),
        aiAnalysisReport: null,
        aiAnalysisTime: null,
        visitLogs: [
        ],
        chatRecords: [
        ],
      ),
      Clue(
        id: '1789783614360',
        wxNick: '小鱼',
        wxId: 'ranran455666',
        phone: '',
        grade: '25级',
        school: '河南轻工',
        subject: '美术专业综合',
        source: '小红书',
        classType: '全程非协议班',
        status: ClueStatus.following,
        intentLevel: IntentLevel.medium,
        tags: const [],
        nextVisitTime: DateTime.parse('2026-09-22T10:00:00.000'),
        ownerName: '李广东',
        createTime: DateTime.parse('2026-09-19T10:06:54.360805'),
        aiAnalysisReport: null,
        aiAnalysisTime: null,
        visitLogs: [
        ],
        chatRecords: [
        ],
      ),
      Clue(
        id: '1789782969895',
        wxNick: '温柔可爱',
        wxId: '',
        phone: '',
        grade: '25级',
        school: '',
        subject: '管理学',
        source: '小红书',
        classType: '全程非协议班',
        status: ClueStatus.contacted,
        intentLevel: IntentLevel.medium,
        tags: const [],
        nextVisitTime: DateTime.parse('2026-09-22T14:30:00.000'),
        ownerName: '超级管理员',
        createTime: DateTime.parse('2026-09-19T09:56:09.895'),
        aiAnalysisReport: null,
        aiAnalysisTime: null,
        visitLogs: [
          VisitLog(
            id: '1789808427916',
            clueId: '1789782969895',
            contactMethod: ContactMethod.wechat,
            visitResult: VisitResult.normal,
            visitContent: r'''还在跟进，约中秋节后的课吧''',
            createTime: DateTime.parse('2026-09-19T17:00:27.916'),
          ),
        ],
        chatRecords: [
        ],
      ),
      Clue(
        id: '1789782678573',
        wxNick: '林建',
        wxId: '',
        phone: '',
        grade: '25级',
        school: '林州建筑',
        subject: '美术专业综合',
        source: '微信',
        classType: '全程非协议班',
        status: ClueStatus.contacted,
        intentLevel: IntentLevel.medium,
        tags: const ["打算报网课"],
        nextVisitTime: DateTime.parse('2026-09-23T14:30:00.000'),
        ownerName: '超级管理员',
        createTime: DateTime.parse('2026-09-19T09:51:18.573'),
        aiAnalysisReport: r'''# 专升本金牌销售总监 · 学员深度诊断报告

## 📌 学员基本画像与核心诉求
- **学员姓名**：林建 | **意向年级**：25届（预科起步，大专25年毕业）
- **就读院校**：林州建筑职业技术学院 | **报考意向**：美术专业综合
- **意向等级**：🔥 **高意向促成期**（学员主动表示“我现在就可以跟着决定一直接受”，报名心智极强）
- **核心关切点**：课程服务周期（是否全程管到明年考前）、集训营频次与教学保障。

---

## 💡 心理防线与顾虑深度剖析
1. **预科生心理**：担心自己基础薄弱或学制特殊，渴望得到机构长期、连续、稳定的教学陪伴。
2. **性价比与全程保障**：学员特别确认“今年报是否教到明年考试”，说明极为看重“一次交费、全程保障到考前”的承诺，只要打消中途二次收费或断档顾虑，即可瞬间促单！

---

## 🎯 针对性实战破冰与逼单策略

### 第一步：打消顾虑，给出定心丸
> “林同学，你放心！咱们这个班型就是专门为你这种25届考生定制的【全程早鸟直通协议班】。你现在定下来，从本月的微集训营开始，一直到你明年4月份全省统考进考场前，中间所有的周末面授、暑假集训、考前冲刺密卷全包在内，绝不会有任何二次收费！”

### 第二步：同校/同专业榜样刺激（标杆案例）
> “你们林州建筑上一届美术专业综合的张学长，也是跟你一样从预科阶段就提前跟咱们微集训营打基础，今年专业课考了全省前50名直接公办本科上岸！美术这种手头功夫，越早跟名师把调子立住，后面文化课压力就越小。”

### 第三步：限时名额与福利逼单
> “今天正好是咱们本月微集训营学员建档的最后一天，主教老师下午就要定下周的面授名额。我现在直接帮你锁定一个早鸟助学名额，把全套内部高分范画集和画具大礼包给你寄到学校，咱们这周末就可以正式进班跟学，你把名字和收件地址发我，我现在就给你办！”

---

## 📅 下一步跟进建议
- **跟进时间**：2026-09-23 14:30（微信确认画具教材寄出与周末集训排课）
- **目标**：锁定定金/全款报名，完成协议签订。
''',
        aiAnalysisTime: DateTime.parse('2026-09-20T21:22:00.000'),
        visitLogs: [
          VisitLog(
            id: 'log_linjian_ai_01',
            clueId: '1789782678573',
            contactMethod: ContactMethod.wechat,
            visitResult: VisitResult.normal,
            visitContent: r'''【AI大模型深度诊断】已完成深度剖析，包含破冰、案例与逼单方案，详见AI分析页。''',
            createTime: DateTime.parse('2026-09-20T21:22:00.000'),
            aiReport: r'''# 专升本金牌销售总监 · 学员深度诊断报告

## 📌 学员基本画像与核心诉求
- **学员姓名**：林建 | **意向年级**：25届（预科起步，大专25年毕业）
- **就读院校**：林州建筑职业技术学院 | **报考意向**：美术专业综合
- **意向等级**：🔥 **高意向促成期**（学员主动表示“我现在就可以跟着决定一直接受”，报名心智极强）
- **核心关切点**：课程服务周期（是否全程管到明年考前）、集训营频次与教学保障。

---

## 💡 心理防线与顾虑深度剖析
1. **预科生心理**：担心自己基础薄弱或学制特殊，渴望得到机构长期、连续、稳定的教学陪伴。
2. **性价比与全程保障**：学员特别确认“今年报是否教到明年考试”，说明极为看重“一次交费、全程保障到考前”的承诺，只要打消中途二次收费或断档顾虑，即可瞬间促单！

---

## 🎯 针对性实战破冰与逼单策略

### 第一步：打消顾虑，给出定心丸
> “林同学，你放心！咱们这个班型就是专门为你这种25届考生定制的【全程早鸟直通协议班】。你现在定下来，从本月的微集训营开始，一直到你明年4月份全省统考进考场前，中间所有的周末面授、暑假集训、考前冲刺密卷全包在内，绝不会有任何二次收费！”

### 第二步：同校/同专业榜样刺激（标杆案例）
> “你们林州建筑上一届美术专业综合的张学长，也是跟你一样从预科阶段就提前跟咱们微集训营打基础，今年专业课考了全省前50名直接公办本科上岸！美术这种手头功夫，越早跟名师把调子立住，后面文化课压力就越小。”

### 第三步：限时名额与福利逼单
> “今天正好是咱们本月微集训营学员建档的最后一天，主教老师下午就要定下周的面授名额。我现在直接帮你锁定一个早鸟助学名额，把全套内部高分范画集和画具大礼包给你寄到学校，咱们这周末就可以正式进班跟学，你把名字和收件地址发我，我现在就给你办！”

---

## 📅 下一步跟进建议
- **跟进时间**：2026-09-23 14:30（微信确认画具教材寄出与周末集训排课）
- **目标**：锁定定金/全款报名，完成协议签订。
''',
          ),
          VisitLog(
            id: '1789889657736',
            clueId: '1789782678573',
            contactMethod: ContactMethod.wechat,
            visitResult: VisitResult.normal,
            visitContent: r'''再次跟进网课的事情，微信没有回复！''',
            createTime: DateTime.parse('2026-09-20T15:34:17.736'),
          ),
          VisitLog(
            id: '1789808222317',
            clueId: '1789782678573',
            contactMethod: ContactMethod.wechat,
            visitResult: VisitResult.followUp,
            visitContent: r'''发微信告知周末班和网课已开课，未回复微信''',
            createTime: DateTime.parse('2026-09-19T16:57:02.317'),
          ),
        ],
        chatRecords: [
          ChatRecord(
            id: 'chat_linjian_03',
            clueId: '1789782678573',
            imagePath: 'assets/chat_3.png',
            imageData: '',
            ocrText: r'''老师：每个月有2-4次老师专门的微集训营
学员：那这个教到什么时候，如果我今年报的话是教到我明年考试吗？
老师：对。
学员：我现在就可以跟着决定一直接受。''',
            createTime: DateTime.parse('2026-09-20T21:20:00.000'),
          ),
          ChatRecord(
            id: 'chat_linjian_02',
            clueId: '1789782678573',
            imagePath: 'assets/chat_2.png',
            imageData: '',
            ocrText: r'''老师：约约试听
学员：打听打听来这个学校了''',
            createTime: DateTime.parse('2026-09-20T21:18:00.000'),
          ),
          ChatRecord(
            id: 'chat_linjian_01',
            clueId: '1789782678573',
            imagePath: 'assets/chat_1.png',
            imageData: '',
            ocrText: r'''老师：你是24届还是25届呀？
学员：我是24届预科生 25届毕业。
老师：那你相当于是25届的大专对吧？
学员：对。''',
            createTime: DateTime.parse('2026-09-20T21:15:00.000'),
          ),
        ],
      ),
      Clue(
        id: '1789782569032',
        wxNick: '金闪闪想出去玩儿',
        wxId: '',
        phone: '',
        grade: '25级',
        school: '河南经贸',
        subject: '美术专业综合',
        source: '抖音',
        classType: '全程非协议班',
        status: ClueStatus.contacted,
        intentLevel: IntentLevel.low,
        tags: const [],
        nextVisitTime: DateTime.parse('2026-09-23T14:30:00.000'),
        ownerName: '超级管理员',
        createTime: DateTime.parse('2026-09-19T09:49:29.032'),
        aiAnalysisReport: r'''# 专升本金牌销售总监 · 学员深度诊断报告

## 📌 学员基本画像与核心诉求
- **学员姓名**：金闪闪想出去玩儿 | **意向年级**：25届（专二在读，25年毕业）
- **就读院校**：河南经贸职业学院 | **报考意向**：美术专业综合
- **来源渠道**：抖音短视频引流 | **意向等级**：❄️ **低意向防御期 / 潜在意向培育期**
- **当前跟进断点**：
  1. 9月19日顾问告知同校同专业状元学姐案例，约试听未回；
  2. 9月20日顾问告知国庆正常上课、可拍视频看教室上课环境，依然微信未回。
- **核心关切点**：抖音公域粉丝防备心重，尚未建立人际信任；担心机构真实性，不愿贸然投入时间到校。

---

## 💡 心理防线与顾虑深度剖析
1. **短视频用户防备心理**：学员从抖音平台过来，对顾问销售性质极其敏感，一旦顾问连续单向推课、约试听，学员会产生被逼单的防御抵触，直接采取“已读不回”或“冷处理”；
2. **需要低门槛的价值锚点**：对于防备心重的学员，直接约试听成本过高，必须用“无销售压力、纯专业干货”打破僵局；
3. **同校从众心理**：学员虽未回复经贸学姐案例，但内心对“本校上岸高分学姐的高分卷/笔记”极具天然兴趣，这是最佳切入口！

---

## 🎯 针对性实战破冰与逼单策略

### 第一步：彻底停止无效催单，进行“破冰降压”
> “闪闪同学，这两天看你没回微信，估计在忙学校专业课作业～不用有压力哈！不用特地抽时间跑来校区，老师先把咱们河南经贸上一届美术专升本考全省高分的李学姐，整理的《美术专业综合历年高分范画与色彩避坑指南》PDF先发你一份，你平时在宿舍就能参考对照着画！”

### 第二步：送资料后“轻量提问”，引导自然开口互动
> 发送资料后追问一句：“对了闪闪，经贸今年大二的素描和色彩写生课程，你们现在每周排几节？你感觉自己现在是色彩调色偏灰，还是素描形体结构吃力一些呢？”（通过专业技术探讨替代销售邀约）

### 第三步：视频实景“软性种草”与国庆试听留位
> “正好国庆假期咱们校区有经贸专场的微写生集训营，有几个也是你们学校的同学组团过来体验。我先用微信给你拍一段现场学长学姐画画的实况视频，你先看看名师改画的氛围。如果国庆期间你在郑州，随时过来坐坐喝杯奶茶，名额我先给你保留着！”

---

## 📅 下一步跟进建议
- **跟进时间**：2026-09-23 14:30（微信发送高分范画PDF并进行专业探讨）
- **目标**：打破微信冷漠沉寂，引导学员首次开口互动，建立专业信任度。
''',
        aiAnalysisTime: DateTime.parse('2026-09-20T21:25:00.000'),
        visitLogs: [
          VisitLog(
            id: 'log_jinshanshan_ai_01',
            clueId: '1789782569032',
            contactMethod: ContactMethod.wechat,
            visitResult: VisitResult.normal,
            visitContent: r'''【AI大模型深度诊断】已完成深度剖析，包含破冰降压、资料诱饵与视频探校方案，详见AI分析页。''',
            createTime: DateTime.parse('2026-09-20T21:25:00.000'),
            aiReport: r'''# 专升本金牌销售总监 · 学员深度诊断报告

## 📌 学员基本画像与核心诉求
- **学员姓名**：金闪闪想出去玩儿 | **意向年级**：25届（专二在读，25年毕业）
- **就读院校**：河南经贸职业学院 | **报考意向**：美术专业综合
- **来源渠道**：抖音短视频引流 | **意向等级**：❄️ **低意向防御期 / 潜在意向培育期**
- **当前跟进断点**：
  1. 9月19日顾问告知同校同专业状元学姐案例，约试听未回；
  2. 9月20日顾问告知国庆正常上课、可拍视频看教室上课环境，依然微信未回。
- **核心关切点**：抖音公域粉丝防备心重，尚未建立人际信任；担心机构真实性，不愿贸然投入时间到校。

---

## 💡 心理防线与顾虑深度剖析
1. **短视频用户防备心理**：学员从抖音平台过来，对顾问销售性质极其敏感，一旦顾问连续单向推课、约试听，学员会产生被逼单的防御抵触，直接采取“已读不回”或“冷处理”；
2. **需要低门槛的价值锚点**：对于防备心重的学员，直接约试听成本过高，必须用“无销售压力、纯专业干货”打破僵局；
3. **同校从众心理**：学员虽未回复经贸学姐案例，但内心对“本校上岸高分学姐的高分卷/笔记”极具天然兴趣，这是最佳切入口！

---

## 🎯 针对性实战破冰与逼单策略

### 第一步：彻底停止无效催单，进行“破冰降压”
> “闪闪同学，这两天看你没回微信，估计在忙学校专业课作业～不用有压力哈！不用特地抽时间跑来校区，老师先把咱们河南经贸上一届美术专升本考全省高分的李学姐，整理的《美术专业综合历年高分范画与色彩避坑指南》PDF先发你一份，你平时在宿舍就能参考对照着画！”

### 第二步：送资料后“轻量提问”，引导自然开口互动
> 发送资料后追问一句：“对了闪闪，经贸今年大二的素描和色彩写生课程，你们现在每周排几节？你感觉自己现在是色彩调色偏灰，还是素描形体结构吃力一些呢？”（通过专业技术探讨替代销售邀约）

### 第三步：视频实景“软性种草”与国庆试听留位
> “正好国庆假期咱们校区有经贸专场的微写生集训营，有几个也是你们学校的同学组团过来体验。我先用微信给你拍一段现场学长学姐画画的实况视频，你先看看名师改画的氛围。如果国庆期间你在郑州，随时过来坐坐喝杯奶茶，名额我先给你保留着！”

---

## 📅 下一步跟进建议
- **跟进时间**：2026-09-23 14:30（微信发送高分范画PDF并进行专业探讨）
- **目标**：打破微信冷漠沉寂，引导学员首次开口互动，建立专业信任度。
''',
          ),
          VisitLog(
            id: '1789889721729',
            clueId: '1789782569032',
            contactMethod: ContactMethod.wechat,
            visitResult: VisitResult.normal,
            visitContent: r'''告知国庆节正常上课，如果不方便过来，可以拍视频看教室上课情况和环境，微信未回复！''',
            createTime: DateTime.parse('2026-09-20T15:35:21.729'),
          ),
          VisitLog(
            id: '1789808490185',
            clueId: '1789782569032',
            contactMethod: ContactMethod.wechat,
            visitResult: VisitResult.normal,
            visitContent: r'''已告知上一届状元学姐同一个专业和学校，今天约试听课没回复！''',
            createTime: DateTime.parse('2026-09-19T17:01:30.185'),
          ),
        ],
        chatRecords: [
        ],
      ),
      Clue(
        id: '1789746533827',
        wxNick: '卡厄斯兰那',
        wxId: '',
        phone: '',
        grade: '25级',
        school: '河南物流（平原新区）',
        subject: '美术专业综合',
        source: '微信',
        classType: '全程非协议班',
        status: ClueStatus.attended,
        intentLevel: IntentLevel.high,
        tags: const ["价格敏感", "学费分期"],
        nextVisitTime: DateTime.parse('2026-09-22T14:30:00.000'),
        ownerName: '超级管理员',
        createTime: DateTime.parse('2026-09-18T23:48:53.827'),
        aiAnalysisReport: r'''# 专升本金牌销售总监 · 学员深度诊断报告

## 📌 学员基本画像与核心诉求
- **学员姓名**：卡厄斯兰那 | **意向年级**：25届（专二在读，25年毕业）
- **就读院校**：河南物流职业学院（平原新区） | **报考意向**：美术专业综合
- **意向等级**：🔥 **高意向促成期（已试听，临门一脚）**
- **业务标签**：【价格敏感】、【学费分期】
- **当前跟进断点**：9月19日顾问告知周末班开课，学员明确回复：“需要跟家里商量报名费的事情”。
- **核心关切点**：对机构教学与试听效果高度认可，但全款学费对家庭预算产生一次性压力；学员想报但缺乏说服家长的有力理由。

---

## 💡 心理防线与顾虑深度剖析
1. **“需要跟家里商量”的真实心理**：
   - 真实度 80%：大专生确实没有独立经济来源，大额学费必须父母支持；
   - 顾虑点 20%：学员自身认可课程，但担心父母觉得贵不同意，自己不知道怎么跟父母开口讲清楚这笔投资的必要性；
2. **价格敏感型学员的痛点**：不是不想掏钱，而是害怕“一次性大额付款压力大”以及“父母觉得考不上钱打水漂”；
3. **促单关键**：帮学员给父母算清投资回报账，提供【0利息助学分期】打消一次性支出压力，并提供【试听保障与协议保障】给家长定心丸！

---

## 🎯 针对性实战破冰与逼单策略

### 第一步：理解共情，主动帮学员“赋能说服家长”
> “卡同学，特别理解！专升本培训毕竟是一笔家庭重要支出，跟家里认真商量是非常成熟负责的做法。很多同学跟爸妈沟通时容易只说‘学费多少钱’，爸妈不了解专升本考情就会觉得突然。老师把咱们这期专门针对家长的《25届公办本科升学规划与投资回报表》发你，里面把公办本科学费比民办每年省1.5万、毕业起点等算得清清楚楚，你今晚拿着这个跟爸妈聊，爸妈一定会全力支持你！”

### 第二步：祭出核心杀手锏：【0手续费助学分期】方案
> “而且你放心，咱们机构针对平原新区物流学院的早鸟学员，开通了【高校助学免息分期通道】。全程班学费可以按月分期缴纳，每个月仅需几百元，完全不需要家里一次性拿出大几千甚至上万元，平摊到平时生活费里家里几乎没有经济负担！”

### 第三步：早鸟名额锁定与定金逼单
> “这周末咱们美术综合面授大课就要正式锁定名额建档了，目前班里仅剩最后2个免息分期助学名额。老师先用我的工号特批帮你占住一个名额和分期政策，你今天先交个200元预留定金（如果跟家里最终商量确实有困难开课前随时全额可退），咱们先把分期优惠和开课资料抢到手！”

---

## 📅 下一步跟进建议
- **跟进时间**：2026-09-22 14:30（微信询问与家长沟通进展，主动提供分期方案）
- **目标**：打消学费顾虑，以 200 元定金锁定免息分期名额，完成周末开课建档。
''',
        aiAnalysisTime: DateTime.parse('2026-09-20T21:26:00.000'),
        visitLogs: [
          VisitLog(
            id: 'log_kaesilanna_ai_01',
            clueId: '1789746533827',
            contactMethod: ContactMethod.wechat,
            visitResult: VisitResult.normal,
            visitContent: r'''【AI大模型深度诊断】已完成深度剖析，包含家长沟通赋能、0息分期与200元定金锁位方案，详见AI分析页。''',
            createTime: DateTime.parse('2026-09-20T21:26:00.000'),
            aiReport: r'''# 专升本金牌销售总监 · 学员深度诊断报告

## 📌 学员基本画像与核心诉求
- **学员姓名**：卡厄斯兰那 | **意向年级**：25届（专二在读，25年毕业）
- **就读院校**：河南物流职业学院（平原新区） | **报考意向**：美术专业综合
- **意向等级**：🔥 **高意向促成期（已试听，临门一脚）**
- **业务标签**：【价格敏感】、【学费分期】
- **当前跟进断点**：9月19日顾问告知周末班开课，学员明确回复：“需要跟家里商量报名费的事情”。
- **核心关切点**：对机构教学与试听效果高度认可，但全款学费对家庭预算产生一次性压力；学员想报但缺乏说服家长的有力理由。

---

## 💡 心理防线与顾虑深度剖析
1. **“需要跟家里商量”的真实心理**：
   - 真实度 80%：大专生确实没有独立经济来源，大额学费必须父母支持；
   - 顾虑点 20%：学员自身认可课程，但担心父母觉得贵不同意，自己不知道怎么跟父母开口讲清楚这笔投资的必要性；
2. **价格敏感型学员的痛点**：不是不想掏钱，而是害怕“一次性大额付款压力大”以及“父母觉得考不上钱打水漂”；
3. **促单关键**：帮学员给父母算清投资回报账，提供【0利息助学分期】打消一次性支出压力，并提供【试听保障与协议保障】给家长定心丸！

---

## 🎯 针对性实战破冰与逼单策略

### 第一步：理解共情，主动帮学员“赋能说服家长”
> “卡同学，特别理解！专升本培训毕竟是一笔家庭重要支出，跟家里认真商量是非常成熟负责的做法。很多同学跟爸妈沟通时容易只说‘学费多少钱’，爸妈不了解专升本考情就会觉得突然。老师把咱们这期专门针对家长的《25届公办本科升学规划与投资回报表》发你，里面把公办本科学费比民办每年省1.5万、毕业起点等算得清清楚楚，你今晚拿着这个跟爸妈聊，爸妈一定会全力支持你！”

### 第二步：祭出核心杀手锏：【0手续费助学分期】方案
> “而且你放心，咱们机构针对平原新区物流学院的早鸟学员，开通了【高校助学免息分期通道】。全程班学费可以按月分期缴纳，每个月仅需几百元，完全不需要家里一次性拿出大几千甚至上万元，平摊到平时生活费里家里几乎没有经济负担！”

### 第三步：早鸟名额锁定与定金逼单
> “这周末咱们美术综合面授大课就要正式锁定名额建档了，目前班里仅剩最后2个免息分期助学名额。老师先用我的工号特批帮你占住一个名额和分期政策，你今天先交个200元预留定金（如果跟家里最终商量确实有困难开课前随时全额可退），咱们先把分期优惠和开课资料抢到手！”

---

## 📅 下一步跟进建议
- **跟进时间**：2026-09-22 14:30（微信询问与家长沟通进展，主动提供分期方案）
- **目标**：打消学费顾虑，以 200 元定金锁定免息分期名额，完成周末开课建档。
''',
          ),
          VisitLog(
            id: '1789808283748',
            clueId: '1789746533827',
            contactMethod: ContactMethod.wechat,
            visitResult: VisitResult.normal,
            visitContent: r'''已告知周末班开课，回复是需要跟家里商量报名费的事情！''',
            createTime: DateTime.parse('2026-09-19T16:58:03.748'),
          ),
        ],
        chatRecords: [
        ],
      ),
      Clue(
        id: '1789746438262',
        wxNick: '柟',
        wxId: '',
        phone: '',
        grade: '25级',
        school: '焦作工贸职业学院',
        subject: '高等数学',
        source: '微信',
        classType: '全程非协议班',
        status: ClueStatus.following,
        intentLevel: IntentLevel.high,
        tags: const ["需要实习", "林州老乡"],
        nextVisitTime: DateTime.parse('2026-09-21T10:00:00.000'),
        ownerName: '超级管理员',
        createTime: DateTime.parse('2026-09-18T23:47:18.262'),
        aiAnalysisReport: null,
        aiAnalysisTime: null,
        visitLogs: [
        ],
        chatRecords: [
        ],
      ),
      Clue(
        id: '1789746292333',
        wxNick: '蒋老师学生',
        wxId: '',
        phone: '',
        grade: '25级',
        school: '河南艺术职业学院',
        subject: '美术专业综合',
        source: '转介绍',
        classType: '全程协议班',
        status: ClueStatus.contacted,
        intentLevel: IntentLevel.medium,
        tags: const ["蒋老师推荐", "班长"],
        nextVisitTime: DateTime.parse('2026-09-23T14:30:00.000'),
        ownerName: '超级管理员',
        createTime: DateTime.parse('2026-09-18T23:44:52.333'),
        aiAnalysisReport: null,
        aiAnalysisTime: null,
        visitLogs: [
          VisitLog(
            id: '1789869382013',
            clueId: '1789746292333',
            contactMethod: ContactMethod.wechat,
            visitResult: VisitResult.followUp,
            visitContent: r'''下雨了，约下次去学校见面！''',
            createTime: DateTime.parse('2026-09-20T09:56:22.013'),
          ),
          VisitLog(
            id: '1789799289592',
            clueId: '1789746292333',
            contactMethod: ContactMethod.wechat,
            visitResult: VisitResult.followUp,
            visitContent: r'''约了20号来试听''',
            createTime: DateTime.parse('2026-09-19T14:28:09.592'),
          ),
        ],
        chatRecords: [
        ],
      ),
      Clue(
        id: '1789746157788',
        wxNick: '我',
        wxId: '',
        phone: '',
        grade: '25级',
        school: '轻工学院嵩山路',
        subject: '美术专业综合',
        source: '小红书',
        classType: '全程非协议班',
        status: ClueStatus.following,
        intentLevel: IntentLevel.high,
        tags: const ["考虑专升本"],
        nextVisitTime: DateTime.parse('2026-09-21T10:00:00.000'),
        ownerName: '超级管理员',
        createTime: DateTime.parse('2026-09-18T23:42:37.788'),
        aiAnalysisReport: null,
        aiAnalysisTime: null,
        visitLogs: [
        ],
        chatRecords: [
        ],
      ),
      Clue(
        id: '1789745780981',
        wxNick: '袁睿琪',
        wxId: '',
        phone: '',
        grade: '25级',
        school: '河南工贸',
        subject: '美术专业综合',
        source: '电话打入',
        classType: '全程协议班',
        status: ClueStatus.enrolled,
        intentLevel: IntentLevel.high,
        tags: const ["家长决策"],
        nextVisitTime: DateTime.parse('2026-09-24T10:00:00.000'),
        ownerName: '超级管理员',
        createTime: DateTime.parse('2026-09-18T23:36:20.981'),
        aiAnalysisReport: null,
        aiAnalysisTime: null,
        visitLogs: [
          VisitLog(
            id: '1789745851231',
            clueId: '1789745780981',
            contactMethod: ContactMethod.wechat,
            visitResult: VisitResult.normal,
            visitContent: r'''提前上周末课！''',
            createTime: DateTime.parse('2026-09-18T23:37:31.231'),
          ),
        ],
        chatRecords: [
        ],
      ),
    ];
  }
}
